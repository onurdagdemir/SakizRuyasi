...
public class PlayObjects : MonoBehaviour
{
    ...
    private Transform currentTarget; // Þu anda hedeflenen nesnenin referansý
    public GameObject targettableImage;
    ...
    void HandleInput()
    {
        ...
        {
            // Eðer bir hedef varsa ve þu anda hedeflenen nesne farklýysa
            if (hit.transform != null && hit.transform != currentTarget)
            {
                // Yeni hedefi güncelle ve rengini deðiþtir
                currentTarget = hit.transform;
                targettableImage.SetActive(true);
            }
        }
        else
        {

                targettableImage.SetActive(false);
                currentTarget = null;
